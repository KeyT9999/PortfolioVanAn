...
export default function ProjectDetail({ params }: { params: { slug: string } }) {
  ...
  if (!project) {
      return (
        <main className="pt-24 pb-32 bg-[#fffae7] text-[#1c1c18] min-h-screen flex flex-col">
  ...
  return (
    <main className="pt-24 bg-[#fffae7] text-[#1c1c18] min-h-screen flex flex-col">
      <Navigation title="MC Vân An" />
      ...
      <section className="flex-grow px-6 md:px-12 max-w-screen-2xl mx-auto w-full mb-32">
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
          {images.map((img, idx) => (
            <div key={idx} className="break-inside-avoid overflow-hidden rounded-sm bg-[#fffae7] relative group shadow-[0_2px_10px_rgba(0,0,0,0.03)] hover:shadow-lg transition-shadow duration-500">
              <img 
        ...
        <div className="mt-24 text-center">
            <Link className="inline-flex items-center justify-center px-10 py-4 border border-[#0a4275]/30 text-[#1c1c18] font-label text-sm tracking-widest uppercase hover:bg-[#fffae7] transition-colors" href="/mc">
